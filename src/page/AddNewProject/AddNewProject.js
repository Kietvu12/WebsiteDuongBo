...
const DocThongMinh = ({ onClose } = {}) => {
    ...
    const [previewUrl, setPreviewUrl] = useState(null); // Tách preview URL riêng
    ...
    const handleFileUpload = (event) => {
        ...
        if (file) {
            ...
            setUserInput(file); // Lưu trực tiếp file vào userInput
    ...
    const handleReadFile = async () => {
        ...
        try {
            ...
            formData.append('image', userInput); // Thêm file vào FormData
            formData.append('message', "Tách ảnh thành chữ cho tôi");
            formData.append('conversation_id', "Kiet");

            const response = await fetch('http://localhost:5000/api/read-image', {
            method: 'POST',
            body: formData // KHÔNG đặt header Content-Type, browser sẽ tự làm
            });
            ...
            setError(err.message || 'Lỗi khi gửi ảnh lên server');
        } finally {
            setIsLoading(false);
        }
        };
    ...
    return (
        ...
        <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[80vh] overflow-auto">
            ...
            {/* Hiển thị khi có file được tải lên */}
            {userInput && (
            <div className="border border-gray-200 rounded-lg p-4 mb-4">
                {previewUrl && (
                    <div className="mb-4">
                        ...
                        <div className="mt-2 text-sm text-gray-600">
                            {userInput.name} - {(userInput.size / 1024).toFixed(2)} KB
                        </div>
                    </div>
                )}
                ...
                <div className="w-8 h-8 md:w-10 md:h-10 mr-2 md:mr-4 flex items-center justify-center">
                    <FileIcon extension={userInput.name.split('.').pop().toLowerCase()} />
                </div>
                <div className="flex-1 min-w-0">
                    <div className="text-xs md:text-sm text-gray-800 text-left truncate">{userInput.name}</div>
                </div>
                </button>
                
                {/* Hiển thị kết quả API */}
                {apiResponse && 
                    console.log('Kết quả từ API:', apiResponse)
                }
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                ...
                <div className="flex flex-col items-center justify-center">
                    ...
                    <span className="text-gray-600 mb-2">Tải ảnh lên</span>
